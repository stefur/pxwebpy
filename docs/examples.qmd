...
From here we can also browse around and get data. Checking out all tables available is doable with `~~.PxDatabase.all_tables()`, but probably a bit overwhelming.

```{python}
db.all_tables()
```

Tables are organised into subjects and are categorised into different `paths`. To see all paths available, use `~~.PxDatabase.get_paths()`.
...
To get all tables that are in specific path you can use `~~.PxDatabase.tables_on_path()`. Here we take a closer look at "Folkmängd".

```{python}
db.tables_on_path(path_id="BE0101A")
```
