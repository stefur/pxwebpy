...
Install the dependency group needed for running tests:
```bash
uv sync --group=tests
```
...
Install the dependency group needed to render the documentation:
```bash
uv sync --group=docs
```

With `quarto` and the dependencies installed the documentation can be rendered:
